...
class BiLSTM:
    ...
    def __init__(self, feat_size, n_classes, device='/cpu:0'):
        tf.reset_default_graph()
        self._device = device
        with tf.device(device):
            self.x = tf.placeholder("float", [None, None, feat_size], 'word_repr/word_embeddings')
    ...
    def add_bidirectional_lstm(self, n_hidden=30, num_layers=3):
        ...
        for layer_num in range(1, num_layers + 1):
            # Define a lstm cell - forward direction cell
        ...
        self.bi_lstm = BiLSTM.fully_connected_layer(outputs, self.n_classes)
